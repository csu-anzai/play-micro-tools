...
trait TryDecorator[T] {
  def apply(block: => BusinessTry[T]): BusinessTry[T]
  def andThen(other: TryDecorator[T]): TryDecorator[T] = new CombinedTryDecorator[T](this, other)
}

class CombinedTryDecorator[T](first: TryDecorator[T], second: TryDecorator[T])
    extends TryDecorator[T] {
