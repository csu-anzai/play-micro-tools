...
trait Retries {
  ...
  def retryFuture[T](
      ...
      errorHandler: PartialFunction[(Throwable, Int), Future[T]] =
        PartialFunction.empty)(implicit ec: ExecutionContext,
                               ctx: LoggingContext,
      ...
      override def apply(block: => Future[T]): Future[T] = {
        Retries.materialize(block).recoverWith {
          ...
          case e if maxRetries > 0 =>
            ...
            after(delay, s)(
                retryFuture(maxRetries - 1, delay, errorHandler).apply(block))
          case e: Throwable =>
  ...
  def retryTry[T](
      maxRetries: Int,
      delay: FiniteDuration,
      problemHandler: PartialFunction[(Problem, Int), BusinessTry[T]] =
        PartialFunction.empty,
      errorHandler: PartialFunction[(Throwable, Int), BusinessTry[T]] =
        PartialFunction.empty)(implicit ec: ExecutionContext,
                               ctx: LoggingContext,
    ...
    new TryDecorator[T] {
      override def apply(block: => BusinessTry[T]): BusinessTry[T] = {
        BusinessTry.future(
            ...
            case Right(problem)
                if problemHandler.isDefinedAt(problem, maxRetries) =>
