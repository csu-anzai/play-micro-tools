...
trait TimedCalls {
  ...
  def timeFuture[T](callId: String)(
  ...
  def timeFuture[T](callId: String, timer: Timer)(
      implicit ec: ExecutionContext,
      ctx: LoggingContext): FutureDecorator[T] = new FutureDecorator[T] {
    override def apply(block: => Future[T]): Future[T] = {
      ...
      result.onComplete {
        case Success(_) =>
          ...
          log.info(s"$callId: Success",
                   "millis" -> (nanos / 1000000.0).toString)
        case Failure(e) =>
          val nanos = timeCtx.stop()
          log.error(s"$callId: Internal error",
                    e,
                    "millis" -> (nanos / 1000000.0).toString)
      }
  ...
  def timeTry[T](callId: String, timer: Timer)(
      implicit ec: ExecutionContext,
      ctx: LoggingContext): TryDecorator[T] = new TryDecorator[T] {
    override def apply(block: => BusinessTry[T]): BusinessTry[T] = {
      ...
      result.onComplete {
        ...
        case Success(BusinessFailure(problem)) =>
          val nanos = timeCtx.stop()
          log.info(s"$callId: Problem: $problem",
                   "millis" -> (nanos / 1000000.0).toString)
        case Failure(e) =>
          val nanos = timeCtx.stop()
          log.error(s"$callId: Internal error",
